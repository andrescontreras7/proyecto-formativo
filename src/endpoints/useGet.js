...
export const getEvaluacionesPorGrupoAsigDocente = async (auth, idasig,idgrupo,idfunc ) => {

  try {
    const response = await fetch(`http://localhost:3001/appi/evaluaciones_del_docente/${idasig}/${idgrupo}/${idfunc}` , {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${auth.token}`
...
export const getNotaEstudiante = async (auth, estudianteId) => {
  try {
    const response = await fetch(`http://localhost:3001/appi/estudiante/notas/${estudianteId}` , {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${auth.token}`
...
export const getActividadUsers = async (month, id) => {
  try {
...
export const getActividadUserByid = async (month, id) => {
  try {
    const response = await fetch(`http://localhost:3001/user_activityBy/${id}?month=${month}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
