...
function ActividadDetalle() {
  ...
  useEffect(() => {
    getEvaluacionesPorId(auth, id).then(res => {
      setActividad(res);
    });
    getEvaluacionesEstudiantes(auth, id).then(res => {
  ...
  const handleCrear = async (idActividad, notaActividad) => {
    try {
      ...
      const data = await getEvaluacionesEstudiantes(auth, id);
          ...
          <table className="w-full text-left table-auto border-collapse border border-gray-200">
            <thead>
              <tr>
                ...
                <th className="border border-gray-200 px-4 py-2">NombreEstudiante</th>
                <th className="border border-gray-200 px-4 py-2">GrupoEstudiante</th>
                <th className="border border-gray-200 px-4 py-2">CursoEstudiante</th>
              ...
              {Array.isArray(actividadEstudiantes) ? (
                actividadEstudiantes.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="border border-gray-200 px-4 py-2">P</td>
                    <td className="border border-gray-200 px-4 py-2">{actividad.id_grupoFK}</td>
                    <td className="border border-gray-200 px-4 py-2">P</td>
                    <td className="border border-gray-200 px-4 py-2">{item.descripcion}</td>
                    <td className="border border-gray-200 px-4 py-2"><a href="">URL</a></td>
